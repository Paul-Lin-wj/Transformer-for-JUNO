...
class EvtDataset(Dataset):
    ...
    def prepare_data(self):
        ...
        for count in range(navi_tree.GetEntries()):
            ...
            # if count < 5:  # Print first few PMT timestamps for debugging
                #     print(f"PMT event {count} timestamp: {navi_evt_t}")
    ...
    def TT_muon_map_construct(self, track_user_file):
        ...
        for count in range(track_tree.GetEntries()):
            track_tree.GetEntry(count)
            # Check if we have coefficients with 2 elements (indicating 2 tracks)
            if hasattr(track_tree, 'nCoeff0'):
                n_track = track_tree.nCoeff0
            else:
                # Assume 2 tracks if Coeff0 has 2 elements
                n_track = len(track_tree.Coeff0) if hasattr(track_tree.Coeff0, '__len__') else 1
            n_points = 6  # track_tree.NTotPoints
            ...
            if hasattr(track_tree, 'start_TS'):
                ...
                continue
            # print(f"Track timestamp: {track_t}, n_track: {n_track}, n_points: {n_points}")
            # if hasattr(track_tree, 'Coeff0'):
            #     print(f"Coeff0 length: {len(track_tree.Coeff0)}")

            # Check if we have multiple tracks or single track
            if n_track >= 2 and n_points >= 6:
                # Process two tracks
                # Extract Coeff values and convert to Python scalars
                # Track 1 (first track)
                ref_pos = [float(track_tree.Coeff0[0]), float(track_tree.Coeff1[0]), float(track_tree.Coeff2[0])]
                direction = [float(track_tree.Coeff3[0]), float(track_tree.Coeff4[0]), float(track_tree.Coeff5[0])]

                # Track 2 (second track)
                ref_pos2 = [float(track_tree.Coeff0[1]), float(track_tree.Coeff1[1]), float(track_tree.Coeff2[1])]
                direction2 = [float(track_tree.Coeff3[1]), float(track_tree.Coeff4[1]), float(track_tree.Coeff5[1])]

                # print(f"Track 1 - ref_pos: {ref_pos}, direction: {direction}")
                # print(f"Track 2 - ref_pos: {ref_pos2}, direction: {direction2}")
                ...
                intersection2_1, intersection2_2 = self.fuc.compute_track_points(ref_pos2, direction2)

                # print(f"Track 1 intersections: {intersection1}, {intersection2}")
                # print(f"Track 2 intersections: {intersection2_1}, {intersection2_2}")

                  # Use Coeff values directly for track endpoints
                # Track 1 entry point
                intersection1 = (
                    float(track_tree.Coeff0[0]),
                    float(track_tree.Coeff1[0]),
                    float(track_tree.Coeff2[0])
                )
                # Track 1 exit point (entry + direction)
                intersection2 = (
                    float(track_tree.Coeff0[0]) + float(track_tree.Coeff3[0]),
                    float(track_tree.Coeff1[0]) + float(track_tree.Coeff4[0]),
                    float(track_tree.Coeff2[0]) + float(track_tree.Coeff5[0])
                )

                # Track 2 entry point
                intersection2_1 = (
                    float(track_tree.Coeff0[1]),
                    float(track_tree.Coeff1[1]),
                    float(track_tree.Coeff2[1])
                )
                # Track 2 exit point (entry + direction)
                intersection2_2 = (
                    float(track_tree.Coeff0[1]) + float(track_tree.Coeff3[1]),
                    float(track_tree.Coeff1[1]) + float(track_tree.Coeff4[1]),
                    float(track_tree.Coeff2[1]) + float(track_tree.Coeff5[1])
                )

              
                # Store both tracks' information:
                # Track 1: entry/exit points (indices 0-5)
                # Track 2: entry/exit points (indices 6-11)
                muon_track_map[track_t] = (
                    intersection1[0],  # Track 1 entry x
                    intersection1[1],  # Track 1 entry y
                    intersection1[2],  # Track 1 entry z
                    intersection2[0],  # Track 1 exit x
                    intersection2[1],  # Track 1 exit y
                    intersection2[2],  # Track 1 exit z
                    intersection2_1[0],  # Track 2 entry x (was a)
                    intersection2_1[1],  # Track 2 entry y (was b)
                    intersection2_1[2],  # Track 2 entry z (was c)
                    intersection2_2[0],  # Track 2 exit x (was d)
                    intersection2_2[1],  # Track 2 exit y (was e)
                    intersection2_2[2]   # Track 2 exit z (was f)
                )
                correlation_map[track_t] = []

                # print(f"Track 1 entry: ({intersection1[0]:.2f}, {intersection1[1]:.2f}, {intersection1[2]:.2f})")
                # print(f"Track 2 entry: ({intersection2_1[0]:.2f}, {intersection2_1[1]:.2f}, {intersection2_1[2]:.2f})")
