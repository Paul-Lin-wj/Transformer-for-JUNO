...
from dsa_algorithm import DSAModule
...
# ============================Transformer with DSA============================


class DSAMultiheadAttention(nn.Module):
    """Multi-head attention with DSA (Dynamic Sparse Attention)"""

    def __init__(self, embed_dim, num_heads, dropout=0.01, dsa_config=None):
        super().__init__()
        assert (
            embed_dim % num_heads == 0
        ), "Embedding dimension must be divisible by the number of heads"
        ...
        # DSA configuration
        self.use_dsa = dsa_config is not None and dsa_config.get('enabled', False)

        if self.use_dsa:
            self.dsa_module = DSAModule(
                embed_dim=embed_dim,
                num_heads=num_heads,
    ...
    def forward(self, Q, K, V, mask=None):
        if self.use_dsa:
            # Use DSA attention
            output, attention_weights, sparsity_stats = self.dsa_module(
...
class TransformerBlockWithDSA(nn.Module):
    """Transformer block with optional DSA attention"""

    def __init__(self, embed_dim, num_heads, ff_dim, dropout=0.1, dsa_config=None):
        super().__init__()
        self.attn = DSAMultiheadAttention(embed_dim, num_heads, dropout, dsa_config)
        self.norm1 = nn.LayerNorm(embed_dim)
        self.ff = FeedForward(embed_dim, ff_dim, dropout)
        self.norm2 = nn.LayerNorm(embed_dim)
        self.use_dsa = dsa_config is not None and dsa_config.get('enabled', False)
    ...
    def forward(self, x, mask=None):
        if self.use_dsa:
            # With DSA, we might want to track sparsity statistics
            h = self.norm1(x + self.attn(x, x, x, mask))
        else:
...
class TransformerWithDSA(nn.Module):
    """
    Transformer model with DSA (Dynamic Sparse Attention) support

    This model can operate in two modes:
    1. Standard mode: Uses regular multi-head attention
    2. DSA mode: Uses Dynamic Sparse Attention for efficiency
    """
    ...
    def __init__(
        ...
        # Transformer blocks with optional DSA
        self.blocks = nn.ModuleList(
            [
                TransformerBlockWithDSA(embed_dim, num_heads, ff_dim, dropout, dsa_config)
                for _ in range(num_layers)
            ]
        )
        ...
        # Store DSA config for potential use in training
    ...
    def get_dsa_stats(self):
        """Get DSA statistics if DSA is enabled"""
        if not self.use_dsa:
            return {"dsa_enabled": False}

        stats = {"dsa_enabled": True}
        for i, block in enumerate(self.blocks):
            if hasattr(block.attn, 'dsa_module'):
                dsa_module = block.attn.dsa_module
                stats[f"block_{i}"] = {
                    "current_sparsity": dsa_module.sparsity_scheduler.get_current_sparsity(),
                    "importance_weight": dsa_module.importance_weight.item(),
                    "temperature": dsa_module.temperature.item()
                }
        return stats

    def update_dsa_sparsity(self, epoch_loss):
        """Update DSA sparsity based on training progress"""
        if self.use_dsa:
            for block in self.blocks:
                if hasattr(block.attn, 'dsa_module'):
                    block.attn.dsa_module.update_sparsity(epoch_loss)
